...
import os
import asyncio
from graph.alpaca_agent import stream_response, get_response

def test_direct_query(query):
    """
    Test the alpaca agent directly without going through the FastAPI server.
    
    Args:
        query (str): The trading query to send to the agent
    """
...
def test_sync_query(query):
    """
    Test the alpaca agent directly using the synchronous get_response function.
...
def run_trading_tests():
    """
    Run a series of predefined test queries for the trading agent.
    """
    # Test cases for Trading Agent
    TEST_QUERIES = [
    ...
    print("Starting Trading Agent Tests")
    print("=" * 70)
...
if __name__ == "__main__":
    # Run the trading tests automatically
    run_trading_tests() 